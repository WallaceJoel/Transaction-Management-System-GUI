...
public class AccountDatabase {
    ...
    * 
    * @param textArea the text Area for gui design
    */
   private void printStatement(TextArea textArea) {
      for (int i = 0; i < size; i++) {
         textArea.appendText("\n" + accounts[i].toString() + "\n");
         textArea.appendText("-interest: $ " + format(accounts[i].monthlyInterest()) + "\n");
         textArea.appendText("-fee: $ " + format(accounts[i].monthlyFee()) + "\n");
         accounts[i].statementTransaction();
         textArea.appendText("-new balance: $ " + format(accounts[i].getBalance()) + "\n");
      }
   }
    ...
    * @param textArea the text Area for gui design
    */
   public void printByDateOpen(TextArea textArea) {
      sortByDateOpen();
      printStatement(textArea);
   }
    ...
    * @param textArea the text Area for gui design
    */
   public void printByLastName(TextArea textArea) {
      sortByLastName();
      printStatement(textArea);
   }
    ...
    * @param textArea the text Area for gui design
    */
   public void printAccounts(TextArea textArea) {
      for (int i = 0; i < size; i++)
   ...
   private String exportFormat(Account account) {
      String[] names = account.getProfile().toString().split(" ");
      if (account.getBank().equals("Checking")) {
         String dd = "false";
         if (account.uniqueAttribute() == 1)
            dd = "true";
         return "C," + names[0] + "," + names[1] + "," + format(account.getBalance()).replace(",", "") + ","
               + account.getDate().toString() + "," + dd + "\n";
      } else if (account.getBank().equals("Savings")) {
         String loyal = "false";
         if (account.uniqueAttribute() == 1)
            loyal = "true";
         return "S," + names[0] + "," + names[1] + "," + format(account.getBalance()).replace(",", "") + ","
               + account.getDate().toString() + "," + loyal + "\n";
      } else
         return "M," + names[0] + "," + names[1] + "," + format(account.getBalance()).replace(",", "") + ","
               + account.getDate().toString() + "," + account.uniqueAttribute() + "\n";
   }
   ...
   public boolean exportFile(File file) {
      try {
         ...
         for (int i = 0; i < size; i++) {
            fileWriter.write(exportFormat(accounts[i]));
         }
         fileWriter.close();
